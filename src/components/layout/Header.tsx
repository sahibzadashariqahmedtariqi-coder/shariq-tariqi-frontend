...
import { Menu, X, Moon, Sun, Heart, Settings, GraduationCap } from 'lucide-react'
      ...
      <div className="container mx-auto px-2 sm:px-4">
        <div className="flex h-14 sm:h-16 items-center justify-between gap-2">
          ...
          <Link to="/" className="flex items-center gap-1 sm:gap-1.5 lg:gap-2 flex-shrink-0 min-w-0" onClick={closeMenu}>
            <img src="https://res.cloudinary.com/du7qzhimu/image/upload/v1771502643/shariq-website/products/tarv5nl0paqk3nehw7ou.png" alt="Logo" className="h-8 w-8 sm:h-10 sm:w-10 lg:h-14 lg:w-14 flex-shrink-0" />
          ...
          <div className="flex items-center space-x-2 lg:space-x-3 flex-shrink-0">
            ...
            {isAuthenticated ? (
              <div className="hidden md:flex items-center space-x-2 flex-shrink-0">
                {/* LMS Button for Users (Students) - show if not admin */}
                {user?.role !== 'admin' && (
                  <Link to="/lms" className="flex items-center space-x-1.5 text-xs lg:text-sm font-medium text-gray-200 transition-colors hover:text-gold-400 whitespace-nowrap bg-primary-800 px-3 py-1.5 rounded-lg">
        ...
        {isMenuOpen && (
          <nav className="md:hidden pb-4 space-y-2 bg-primary-900">
            ...
            <div className="pt-4 border-t border-primary-700 space-y-2">
              ...
              {isAuthenticated ? (
                <>
                  {/* LMS Button for Users (Students) - Mobile - show if not admin */}
                  {user?.role !== 'admin' && (
                    <Link to="/lms" onClick={closeMenu}>
